...
for i in $(seq 1 $CLUSTER);do
   ...
   oc exec -it $pod -- sh -c "id=$(/opt/keycloak/bin/kcadm.sh get clients -q clientId=myclient-$i -r myrealm-$i --fields id --format csv --noquotes --no-config --server http://localhost:8080 --realm master --user admin --password admin)"
   oc exec -it $pod -- sh -c "/opt/keycloak/bin/kcadm.sh update clients/$id -s 'redirectUris=["https://oauth-openshift.apps.sno-$i.sandbox2471.opentlc.com/oauth2callback/keycloak/*"]' -s 'directAccessGrantsEnabled=true' -r myrealm --no-config --server http://localhost:8080 --realm master --user admin --password admin"
